...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetFireArmsQuery>());
    ...
    {
        var result = await Mediatr.Send(request.ToQuery<GetFireArmsQuery>(nameof(FireArmsState.ItemDescription)));
    ...
    {        
        return await BatchUploadAsync<IndexModel, FireArmsState>(BatchUpload.BatchUploadForm, nameof(FireArmsState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<FireArmsState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
