...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetFireArmsBrandModelQuery>());
    ...
    {
        var result = await Mediatr.Send(request.ToQuery<GetFireArmsBrandModelQuery>(nameof(FireArmsBrandModelState.Description)));
    ...
    {        
        return await BatchUploadAsync<IndexModel, FireArmsBrandModelState>(BatchUpload.BatchUploadForm, nameof(FireArmsBrandModelState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<FireArmsBrandModelState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
