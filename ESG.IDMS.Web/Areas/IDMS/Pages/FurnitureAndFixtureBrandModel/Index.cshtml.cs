...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetFurnitureAndFixtureBrandModelQuery>());
    ...
    {
        var result = await Mediatr.Send(request.ToQuery<GetFurnitureAndFixtureBrandModelQuery>(nameof(FurnitureAndFixtureBrandModelState.Id)));
        return new JsonResult(result.ToSelect2Response(e => new Select2Result { Id = e.Id, Text = e.Id }));
    }
	public async Task<IActionResult> OnPostBatchUploadAsync()
    {        
        return await BatchUploadAsync<IndexModel, FurnitureAndFixtureBrandModelState>(BatchUpload.BatchUploadForm, nameof(FurnitureAndFixtureBrandModelState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<FurnitureAndFixtureBrandModelState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
