...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetOtherEquipmentBrandModelQuery>());
    ...
    {
        var result = await Mediatr.Send(request.ToQuery<GetOtherEquipmentBrandModelQuery>(nameof(OtherEquipmentBrandModelState.Id)));
        return new JsonResult(result.ToSelect2Response(e => new Select2Result { Id = e.Id, Text = e.Id }));
    }
	public async Task<IActionResult> OnPostBatchUploadAsync()
    {        
        return await BatchUploadAsync<IndexModel, OtherEquipmentBrandModelState>(BatchUpload.BatchUploadForm, nameof(OtherEquipmentBrandModelState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<OtherEquipmentBrandModelState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
