...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetRadioEquipmentQuery>());
    ...
    {
        var result = await Mediatr.Send(request.ToQuery<GetRadioEquipmentQuery>(nameof(RadioEquipmentState.ItemDescription)));
        return new JsonResult(result.ToSelect2Response(e => new Select2Result { Id = e.Id, Text = e.ItemDescription }));
    }
	public async Task<IActionResult> OnPostBatchUploadAsync()
    {        
        return await BatchUploadAsync<IndexModel, RadioEquipmentState>(BatchUpload.BatchUploadForm, nameof(RadioEquipmentState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<RadioEquipmentState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
