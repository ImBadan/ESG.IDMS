...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetRadioEquipmentBrandModelQuery>());
    ...
    {
        var result = await Mediatr.Send(request.ToQuery<GetRadioEquipmentBrandModelQuery>(nameof(RadioEquipmentBrandModelState.Description)));
    ...
    {        
        return await BatchUploadAsync<IndexModel, RadioEquipmentBrandModelState>(BatchUpload.BatchUploadForm, nameof(RadioEquipmentBrandModelState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<RadioEquipmentBrandModelState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
