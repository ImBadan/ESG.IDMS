...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetVehicleQuery>());
    ...
    {
        var result = await Mediatr.Send(request.ToQuery<GetVehicleQuery>(nameof(VehicleState.ItemDescription)));
    ...
    {        
        return await BatchUploadAsync<IndexModel, VehicleState>(BatchUpload.BatchUploadForm, nameof(VehicleState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<VehicleState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
